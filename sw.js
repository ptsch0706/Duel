...
// (i.e. genuinely offline). This means deploying a new version of any game
// file is picked up the next time the app is opened with any connectivity
// at all — no more deleting and reinstalling the home screen icon to see
// updates. Same pattern as Focus's service worker.
//
// Trade-off: this app is always network-first, so it does one fetch on every
// open when online (negligible for files this size) in exchange for never
// getting stuck on a stale cached version.

const CACHE_NAME = 'duel-cache-v1';
// Bump CACHE_NAME (e.g. 'duel-cache-v2') only if you want to force a one-time
...
self.addEventListener('fetch', (event) => {
  ...
  // (Firestore, Google Fonts, the dictionary/trivia APIs) pass through
  // untouched — those aren't ours to cache and shouldn't be intercepted.
